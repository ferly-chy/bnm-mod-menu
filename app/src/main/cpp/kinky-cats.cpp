...
void new_CardCharacterCtor(BNM::IL2CPP::Il2CppObject *instance, void *data,
                           void *container) {
...
void OnLoaded() {
  ...
  auto Player = BNM::Class("Cats.Services", "Player", AssemblyCSharp);
  auto AddCurrencySilently = Player.GetMethod("AddCurrencySilently");
  auto Spend = Player.GetMethod("Spend");

  auto CardCharacter =
      BNM::Class("Cats.Characters", "CardCharacter", AssemblyCSharp);

  auto CardCharacterCtor = CardCharacter.GetMethod(".ctor");

  BNM::BasicHook(AddCurrencySilently, new_AddCurrencySilently,
                 old_AddCurrencySilently);
  BNM::BasicHook(Spend, new_Spend, old_Spend);
  BNM::BasicHook(CardCharacterCtor, new_CardCharacterCtor,
                 old_CardCharacterCtor);
}
