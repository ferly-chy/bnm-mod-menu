...
Java_com_android_support_Menu_getFeatureList(JNIEnv *env, jobject thiz) {
  std::string feats[] = {"Toggle:Currencies", "Toggle:Characters",
                         "Seekbar:Reward:1_20"};
...
void (*old_AddResource)(void *instance, int resourceType, int amount,
                        int reason, int rewardSource);

void new_AddResource(void *instance, int resourceType, int amount, int reason,
                     int rewardSource) {
  return old_AddResource(instance, resourceType, amount * feature.reward,
                         reason, rewardSource);
}

bool (*old_TrySubtractResource)(void *instance, int resourceType, int amount,
                                int reason, void *reasonDetails);

bool new_TrySubtractResource(void *instance, int resourceType, int amount,
                             int reason, void *reasonDetails) {
...
void new_GiveGift(BNM::IL2CPP::Il2CppObject *instance,
                  BNM::IL2CPP::Il2CppObject *gift) {
  if (feature.currencies) {
    auto GiveGift = GetMethod<BNM::IL2CPP::Il2CppObject *>(instance, "GiveGift",
                                                           {"expAmount"});
...
void new_ContactControllerCtor(BNM::IL2CPP::Il2CppObject *instance, void *data,
                               void *profile) {
...
void OnLoaded() {
  ...
  auto ResourcesManager =
      BNM::Class("Game.Scripts.Engine.Profiles.GameResources",
                 "ResourcesManager", AssemblyCSharp);
  auto AddResource = ResourcesManager.GetMethod("AddResource", 4);
  auto TrySubtractResource =
      ResourcesManager.GetMethod("TrySubtractResource", 4);

  auto RelationshipController = BNM::Class(
      "Game.Scripts.Relationship", "RelationshipController", AssemblyCSharp);

  auto GiveGift = RelationshipController.GetMethod("GiveGift", {"gift"});

  auto ContactController =
      BNM::Class("Game.Scripts.Contacts", "ContactController", AssemblyCSharp);

  auto ContactControllerCtor = ContactController.GetMethod(".ctor");

  BNM::BasicHook(AddResource, new_AddResource, old_AddResource);
  BNM::BasicHook(TrySubtractResource, new_TrySubtractResource,
                 old_TrySubtractResource);
  BNM::BasicHook(GiveGift, new_GiveGift, old_GiveGift);
  BNM::BasicHook(ContactControllerCtor, new_ContactControllerCtor,
                 old_ContactControllerCtor);
}
